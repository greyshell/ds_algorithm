...
node *get_node(singly_linked_list *list, int data) {
    /*
...
node *get_kth_node(singly_linked_list *list, size_t k) {
    /*
     * 1 traversal / pass
...
node *get_kth_node_from_end(singly_linked_list *list, size_t k) {
    /*
     * 1 traversal / pass
     * time complexity: O(n)
     * space complexity: O(1)
     */
    node *first_ptr, *second_ptr;
    size_t count;
...
node *get_middle_node(singly_linked_list *list) {
    /*
    ...
    // when the singly_linked_list count >= 2
    while (fast_ptr != NULL && fast_ptr->next != NULL) {
        slow_ptr = slow_ptr->next;
        fast_ptr = fast_ptr->next->next;
    }
    return slow_ptr;  // if the singly_linked_list count is 1 the it returns NULL
}
...
size_t count(singly_linked_list *list) {
    /*
     * time complexity: O(n)
...
int delete_at_tail(singly_linked_list *list) {
     ...
     * space complexity: O(1)
     */
    node *temp_node, *second_last_node;
    int data;
    const size_t k = 2;
...
void delete_element(singly_linked_list *list, int data) {
    node *temp_node, *curr_ptr, *prev_ptr;
